...
public class PreferencesBean implements Serializable {
    ...
    @JsonProperty("numberOfColumnsInGameList")
    private int numberOfColumnsInGameList = 1;
    ...
    @JsonProperty("startWithFloppyFlow")
    ...
    public void setIconSize(String s) {
        ...
        if (matcher.find()) {
            iconWidth = Integer.parseInt(matcher.group(1));
            iconHeight = Integer.parseInt(matcher.group(2));
        } else {
            System.err.println("[ERROR] Invalid icon size format: " + s);
        }
    }

    public int getIconSizeIndex() {
        int[] sizes = {16, 22, 32, 48, 164, 160, 320};
        for (int i = 0; i < sizes.length; i++) {
            if (iconWidth == sizes[i]) return i;
        }
        return -1;
    }
