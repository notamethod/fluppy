...
public class GameActions {
    ...
    private List<Game> findGame(String name, ApiCalls apiCalls) {
        ...
        while ((resp = da.showInputDialog("The title", "Enter the name")).isPresent()) {
...
//
    ...
    private GameApp addArchive(File file) throws GameManagerException {
    ...
    private GameApp addDirectory(File inFile) throws GameManagerException {
        GameApp mgame = new GameApp();
        ...
        if (count == 0) {
            throw new GameManagerException("Can't find any suitable executable files in" + inFile.getAbsolutePath() + ".");
        }
        ...
        if (count > 1) {
            Optional<String> o = da.showListInputDialog(
                    "What's the name of the game?",
                    "You have dropped a directory. What is the main executable?",
                    possible, possible[0]);
            if (o.isPresent()) {
                chosen = o.get();
            }
     ...
     * @return
     */
    public GameApp createFromFile(File inFile) throws GameManagerException {
        //File exeFile = inFile;
        ...
        try {
            ...
            //FIXME
            //d.setGamePath(metaGame.getGamePath().toAbsolutePath().toString());
            ...
            if (guessSource != null) {
                Game me = HelperClass.regexArchive(inFile.getName());
                if (me != null) {
                    choice[0] = me.getName();
                }
            }

            choice[1] = "Something else...";

            String input = da.showListInputDialog(
                    "What is the title of the application?", "Select one of the proposals,\n" +
                            "or select \"Something else...\" to type your own.",

                     choice, choice[0]).orElseGet(()->"");
            ...
            if (game != null) {
                ...
                metaGame.setYear(Integer.valueOf(game.getYear()));
            ...
            if (!da.showConfirmDialog("Adding game " + metaGame.getName() + " to the list ?", "Please Confirm")){
                return null;
            }
